...
function routeAction(action, params) {
  const routes = {
    ...
    'create_spbe': () => actionCreateSPBE(params),
    'update_spbe': () => actionUpdateSPBE(params),
    'delete_spbe': () => actionDeleteSPBE(params),
...
function actionRegisterCompany(params) {
  ...
  try {
    ...
    appendRow(spbeSheet, [
      generateId('SPB'), 'SPBE Default', address || '', phone || '',
      '', 0, 'aktif', 'Data awal — harap diperbarui', new Date().toISOString(), '',
    ]);
...
function actionGetSPBE(params) {
  try {
    ...
    const data  = sheetToObjects(sheet)
      ...
      .map(s => ({
        id:        s.id,
        name:      s.nama,   // kept for dropdown compatibility
        nama:      s.nama,
        alamat:    s.alamat  || '',
        telepon:   s.telepon || '',
        pic:       s.pic     || '',
...
function actionCreateSPBE(params) {
  try {
    const user = params._user;
    if (user.role === 'driver') return { status: 'error', message: 'Tidak ada akses.' };

    const nama = String(params.nama || '').trim();
    if (!nama) return { status: 'error', message: 'Nama SPBE wajib diisi.' };
    ...
    if (existing.some(s => s.nama?.toLowerCase() === nama.toLowerCase())) {
      return { status: 'error', message: `SPBE dengan nama "${nama}" sudah ada.` };
    }
    ...
    appendRow(sheet, [
      ...
      params.telepon  || '',
      params.pic      || '',
      Number(params.kapasitas) || 0,
      params.status   || 'aktif',
      params.catatan  || '',
      new Date().toISOString(),
    ]);

    writeAuditLog(getMasterSS(), user, 'create', `Tambah SPBE: ${nama}`, user.company);
    return { status: 'ok', data: { id } };
  } catch (e) { return { status: 'error', message: e.message }; }
}

function actionUpdateSPBE(params) {
  try {
    const user = params._user;
    if (user.role === 'driver') return { status: 'error', message: 'Tidak ada akses.' };
    ...
    const updates = {};
    ['nama','alamat','telepon','pic','kapasitas','status','catatan'].forEach(k => {
      if (params[k] !== undefined) updates[k] = params[k];
    });
    updates.updated_at = new Date().toISOString();
    // keep 'name' alias in sync
    if (updates.nama) updates.name = updates.nama;

    const ok = updateRowById(sheet, params.id, updates);
    if (!ok) return { status: 'error', message: 'SPBE tidak ditemukan.' };

    writeAuditLog(getMasterSS(), user, 'update',
      `Update SPBE #${params.id}: ${JSON.stringify(updates)}`, user.company);
    return { status: 'ok' };
  } catch (e) { return { status: 'error', message: e.message }; }
}

function actionDeleteSPBE(params) {
  try {
    const user = params._user;
    if (user.role === 'driver') return { status: 'error', message: 'Tidak ada akses.' };
    ...
    writeAuditLog(getMasterSS(), user, 'delete', `Hapus SPBE #${params.id}`, user.company);
    return { status: 'ok' };
  } catch (e) { return { status: 'error', message: e.message }; }
}
...
const HEADERS = {
  ...
  SPBE: ['id','nama','alamat','telepon','pic','kapasitas','status','catatan','created_at','updated_at'],
...
function setupMasterSpreadsheet() {
  ...
  // Create all required Master sheets
  getOrCreateSheet(master, 'Perusahaan', HEADERS.PERUSAHAAN);
  getOrCreateSheet(master, 'Users', HEADERS.USERS);
  getOrCreateSheet(master, 'Sessions', HEADERS.SESSIONS);
  getOrCreateSheet(master, 'AuditLog', HEADERS.AUDIT_LOG);
  getOrCreateSheet(master, 'ErrorLog', ['timestamp','context','message','stack']);

  return 'Setup Master berhasil! Sheet: Perusahaan, Users, Sessions, AuditLog, ErrorLog sudah siap. ' +
    'Daftarkan perusahaan pertama melalui halaman registerpt.html.';
}
...
function setupCompanySpreadsheet(companyCode) {
  try {
    ...
    if (sheetToObjects(spbeSheet).length === 0) {
      appendRow(spbeSheet, ['S001','SPBE Cibiru','Jl. Cibiru No.1','022-11111111','',0,'aktif','',new Date().toISOString(),'']);
      appendRow(spbeSheet, ['S002','SPBE Bandung Barat','Jl. BBS No.5','022-22222222','',0,'aktif','',new Date().toISOString(),'']);
    }
